...
public class UserController {
	...
	public String register(User user, Map model) {
		if (service.register(user)) {
			...
			// Registration failed
			model.put("Info", "Sorry! User already exists with same email");
			return "register";
		}
	}
	...
	public String validate(ForgetBean forget, Map model, HttpSession session) {
		if (service.validate(forget)) {
			...
			return "change"; 	//we don't write change.jsp bcoz we have given prefix and suffix in spring-servlet.xml
		} else {
			// Validation failed
	...
	public String update(LoginBean change, Map model, HttpSession session)
	{
		...
		if(service.update(change)) {
			...
			session.removeAttribute("EMAIL"); //We need not remember the email once the password is changed
			return "index";
		} else {
	...
	public String logout(Map model, HttpSession session) {
		...
		session.removeAttribute("USER"); //We need not remember the user once logged out in a session
		session.invalidate();
		return "index";
	}
}
